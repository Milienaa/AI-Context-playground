            ...
            <ReactMarkdown
                 ...
                 components={{
                    pre: ({node, ...props}) => {
                         ...
                         if (codeNode && 'tagName' in codeNode && codeNode.tagName === 'code' && 'children' in codeNode) {
                             const codeContent = codeNode.children[0];
                             if (codeContent && 'value' in codeContent) {
                                return <CodeBlock content={String(codeContent.value).replace(/\n$/, '')} />;
                             }
                         }
                         return <pre {...props} className="bg-gray-800 p-2 rounded-md" />;
                    },
                    table: ({node, ...props}) => <table {...props} className="w-full text-left border-collapse" />,
                    thead: ({node, ...props}) => <thead {...props} className="bg-gray-800" />,
                    th: ({node, ...props}) => <th {...props} className="border border-gray-600 p-2" />,
                    td: ({node, ...props}) => <td {...props} className="border border-gray-600 p-2" />,
                 }}
            >
                {message.content}
            </ReactMarkdown>
